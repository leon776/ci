...
{	
	...
	//加载模板
	public function load_temp($page, $data = array(), $header = '/common/header', $footer = '/common/footer') {
		$data['lang'] = $this->session->userdata('language');
		$this->load->view($data['lang'].$header, $data);
		foreach ($page as $key => $value) {
			$this->load->view($value);
		}
		$this->load->view($data['lang'].$footer);
	}
	//回到上一页
